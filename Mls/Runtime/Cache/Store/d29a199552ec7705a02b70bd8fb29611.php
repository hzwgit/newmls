...
<body>
    <div style="width:1000px;margin:0px auto">
        <h1>
            <a type="button" class="btn btn-primary btn-lg" href="<?php echo U('Good/index');?>">放回</a>
        </h1>
        ...
        <div id="imgs" style="width:965px;min-height:600px;border:1px solid #eee">
            ...
            <?php if(is_array($goods)): foreach($goods as $key=>$vo): ?><div data-name="image" style="float:left;position:relative" data-id="<?php echo ($vo['g_pic']); ?>
                    ...
                    data-gid="<?php echo ($vo['g_id']); ?>" data-id="<?php echo ($vo['g_pic']); ?>">删除</div>
                        <!--封面-->
                        <div data-name="state" style="position:absolute;top:10px;left:10px;border:1px solid #eee;background:#fff;cursor:pointer;display:none" 
						data-gid="<?php echo ($vo['g_id']); ?>" data-id="<?php echo ($vo['g_pic']); ?>">设为封面</div>
                                <img width='240px' height='240px' style='padding:10px;'  
								src='/Project/Public<?php echo ($vo["g_pic"]); ?>' 
								data-id="<?php echo ($vo['g_pic']); ?>"/> <!--判断是否是选中状态-->
    ...
    $(function() {
	 ...
	 $(document).on('click','div[data-name="del"]',function(){
        ...
        $.get('/Project/index.php/Store/Images/del',{g_pic:$pid,g_id:$g_id},function(data){
          console.log(data);
          if(data != '00'){
            obj.remove();
          } else {
      ...
      $(document).on('click','div[data-name="state"]',function(){
        ...
        var $pid = $(this).attr('data-id');
        var $g_id = $(this).attr('data-gid');
        $.get('/Project/index.php/Store/Images/state',{g_pic:$pid,g_id:$g_id},function(data){
          console.log(data)
          if(data){
      ...
      $('#file_upload').uploadify({
        ...
        'uploader' : '<?php echo U("Images/upload?id=$id");?>',
        'onUploadSuccess' : function(file, data, response) {
			console.log(data);
          var $data = $.parseJSON(data);
          //console.log(data);
             //把所有上传的图片都放入DIV中
             //img = "<img width='240px' height='240px' style='padding:10px;' src='/Project/Public"+data+"' />";
			 
			  img = '<div data-name="image" style="float:left;position:relative"' 
			 +'data-id="' + $data.g_pic + '">'
                    +'<div data-name="del" style="position:absolute;top:10px;right:10px;border:1px solid #eee;background:#fff;cursor:pointer;display:none" '
					+'data-gid="' + $data.g_id + '" '
                    +'data-id="' + $data.g_pic + '">删除</div>'
                    +'    <div data-name="state" style="position:absolute;top:10px;left:10px;border:1px solid #eee;background:#fff;cursor:pointer;display:none" data-gid="' + $data.g_id + '"'
                    +'    data-id="' + $data.g_pic + '">设为封面</div>'
                    +'        <img width="240px" height="240px" style="padding:10px;"'
                    +'        src=" /Project/Public' + $data.g_pic + ' " data-id="' + $data.g_id + '"/>'
                    +'        </div>'
                    +'</div>';

...
<body>
    <div style="width:1000px;margin:0px auto">
        ...
        <div id="imgs" style="width:965px;min-height:600px;border:1px solid #eee">
            ...
            <?php if(is_array($goods)): foreach($goods as $key=>$vo): ?><div data-name="image" style="float:left;position:relative" data-id="<?php echo ($vo['g_pic']); ?>
                    ">
                    <!--删除-->
                    <div data-name="del" style="position:absolute;top:10px;right:10px;border:1px solid #eee;background:#fff;cursor:pointer;display:none" data-gid="<?php echo ($vo['s_id']); ?>" 
                    data-id="<?php echo ($vo['pic']); ?>">删除</div>
                        <!--封面-->
                        <div data-name="state" style="position:absolute;top:10px;left:10px;border:1px solid #eee;background:#fff;cursor:pointer;display:none" data-gid="<?php echo ($vo['s_id']); ?>"
                        data-id="<?php echo ($vo['pic']); ?>">设为封面</div>
                            <img width='930px' height='80px' style='padding:10px;'  
                            src='/Project/Public<?php echo ($vo["pic"]); ?>' data-id="<?php echo ($vo['g_pic']); ?>"/>
                                <!--判断是否是选中状态-->
                                <?php if($vo[state] == '1'): ?><span class="glyphicon glyphicon-ok text-success" aria-hidden="true" style="position:absolute;top:27px;right:15px;bgcolor:green;font-size:40px;"></span><?php endif; ?>
    ...
    $(function() {
	 ...
	 $(document).on('click','div[data-name="del"]',function(){
        ...
        $.get('/Project/index.php/Store/Banner/del',{g_pic:$pid,g_id:$g_id},function(data){
          console.log(data);
          if(data != '00'){
            obj.remove();
          } else {
      ...
      //设为封面
      $(document).on('click','div[data-name="state"]',function(){
        var obj = $(this).parent();
		console.log(obj);
        var $pid = $(this).attr('data-id');
        var $g_id = $(this).attr('data-gid');
        $.get('/Project/index.php/Store/Banner/state',{g_pic:$pid,g_id:$g_id},function(data){
          console.log(data)
          if(data){
            $('div[data-name="state"]').parent().children('span').remove();
            obj.append('<span class="glyphicon glyphicon-ok text-success" aria-hidden="true" style="position:absolute;top:25px;right:15px;bgcolor:green;font-size:40px;"></span>');
          }
        })
      })
      ...
      $('#file_upload').uploadify({
        ...
        'uploader' : '<?php echo U("Banner/upload");?>',
        'onUploadSuccess' : function(file, data, response) {
		  ...
		  //console.log(data.pic);
			 ...
			 +'data-id="' + $data.pic + '">'
                    ...
                    +'        <img width="930p"" height="80px" style="padding:10px;"'

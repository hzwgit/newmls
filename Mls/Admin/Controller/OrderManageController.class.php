...
class OrderManageController extends Controller {
	public function index(){
		...
		//查询条数
		$count=$order
				->join('mls_order_info info  on info.o_id=mls_order.id',"right")
				->join("(select id,name ,discount ,price from  mls_goods)  g  on g.id=info.g_id","right")
				->join('(select g_id,g_pic from mls_good_pic  group by g_id) gpic on gpic.g_id=info.g_id','left')
				->join('(select id as userid ,nickname from mls_user) users on users.userid=info.u_id','right')

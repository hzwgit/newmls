   ...
   var $i = 0;
    ...
    window.onscroll=function(){
        ...
        if(checkscrollside()){
			...
			var $pid = $('#main').attr('pata-pid');
			$i++;
            //ajax 获取数据
			$.ajaxSetup({
				async:false 
			})
			$.get('/project/index.php/Home/Guang/hotWaterPage',{id:$id,pid:$pid,page:$i},function(dataInt){
				//console.log(dataInt)
				if(dataInt){
					data(dataInt);
				}
                ...
                var img = '<div class="pin poster_grid retsop poster_wall">'
                        +'<div class="new_poster">'
                             +'<div class="np_pic hover_pic">'
                                ...
                                +'     ￥' + value.gprice
                                +' </div> '
                                +' <a class="pic_load" href="/Project/index.php/home/Share/index/id/'+value.gid+'" target="_blank" title="'+value.gname+'"><img class="goods_pic" src="/Project/Public'+value.gpic+'" alt="'+value.gname+'" width="226" /></a> '
                            ...
                            +' <div class="clear_f"></div>  '
                             +'<div class="comm_num twiiter_box">  '
                                 +'<a class="pl" href="__MODULE__/Share/index/id/<{$res.gid}>" target="_blank">销量（'+value.gbuy+'）</a>  '
                                 +'<a isshowlike="1" s_uid="85591250" class="left_f poster_likes likes " href="javascript:void(0)"> <b class="likes_status"> <i class="lm_love2">&nbsp;</i>喜欢 </b> <span class="red_f f14_f poster_like_num">110</span> </a>  '
                            +' </div>  '
                             +'<div class="np_share comment_panel">  '
                                 // +'<a class="avatar32_f trans07 shopTips" href="/shop/193784" user_id="193784" target="_blank"><img src="{店铺图片}" /></a>  '
                                 // +'<p class="ml42_f"><a href="{店铺路径}" target="_blank">{店铺}</a></p>  '
